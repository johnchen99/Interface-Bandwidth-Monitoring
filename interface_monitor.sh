...
sleep_time=$(( INTERVAL - $(TZ=Asia/Singapore date +%s) % INTERVAL ))

# Start of transmit 
tx1=$(ifconfig $NETWORK_INTERFACE | awk '/TX packets/{print $5}')

# Wait until the next five-minute interval
#echo "$(TZ=Asia/Singapore date +%Y-%m-%d %H:%M:%S) - Waiting $sleep_time seconds until the next interval..."
...
OUTPUT_DIR=$OUTPUT_DIR/$(TZ=Asia/Singapore date +%Y-%m)
if [ ! -d $OUTPUT_DIR ]; then
    mkdir -p -m 755 $OUTPUT_DIR
fi

# Generate a unique filename based on the current timestamp
filename=$(TZ=Asia/Singapore date +%Y%m%d)_tx.log
...
if find "$OUTPUT_DIR" -type f -mtime +"$DAYS_TO_KEEP" -delete -print; then
    echo "$(TZ=Asia/Singapore date +%Y-%m-%d_%H:%M:%S) - Deleted log files older than 30 days."
fi
# done
